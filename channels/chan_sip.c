...
static void *_sip_tcp_helper_thread(struct sip_pvt *pvt, struct ast_tcptls_session_instance *tcptls_session) 
{
	...
	for (;;) {
		...
		handle_request_do(&req, &tcptls_session->requestor);
	}
...
static void realtime_update_peer(const char *peername, struct sockaddr_in *sin, const char *defaultuser, const char *fullcontact, int expirey, int deprecated_username, int lastms)
{
	...
	ast_update_realtime(tablename, "name", peername, "lastms", str_lastms, NULL);
}
...
static int sip_call(struct ast_channel *ast, char *dest, int timeout)
{
	...
	if (!(p->jointcapability & AST_FORMAT_AUDIO_MASK)) {
		...
		AST_SCHED_REPLACE(p->initid, sched, p->timer_b, auto_congest, p);
	}
...
static struct ast_channel *sip_new(struct sip_pvt *i, int state, const char *title)
{
	...
	if ((ast_test_flag(&i->flags[0], SIP_DTMF) == SIP_DTMF_INBAND) || (ast_test_flag(&i->flags[0], SIP_DTMF) == SIP_DTMF_AUTO) ||
		...
		int features = DSP_FEATURE_DTMF_DETECT;
		...
		ast_dsp_set_features(i->dsp, features);
                if (global_relaxdtmf)
			ast_dsp_digitmode(i->dsp, DSP_DIGITMODE_DTMF | DSP_DIGITMODE_RELAXDTMF);
        }
...
static void destroy_association(struct sip_peer *peer)
{
	...
	if (!sip_cfg.ignore_regexpire) {
		if (peer->rt_fromcontact && sip_cfg.peer_rtupdate) {
			...
			ast_update_realtime(tablename, "name", peer->name, "lastms", "", NULL);
		} else {
...
#define FORMAT  "%-30.30s %-6d %-9.9s %-6.6s\n"
	...
	AST_LIST_TRAVERSE(&threadl, th, list) {
		ast_cli(a->fd, FORMAT, ast_inet_ntoa(th->tcptls_session->requestor.sin_addr), 
			ntohs(th->tcptls_session->requestor.sin_port), 
...
static void handle_response_peerpoke(struct sip_pvt *p, int resp, struct sip_request *req)
{
	...
	if (statechanged) {
		...
		ast_device_state_changed("SIP/%s", peer->name);
		if (sip_cfg.peer_rtupdate) {
			ast_update_realtime(ast_check_realtime("sipregs") ? "sipregs" : "sippeers", "name", peer->name, "lastms", str_lastms, NULL);
		}
...
static struct ast_tcptls_session_instance *sip_tcp_locate(struct sockaddr_in *s)
{
	...
	AST_LIST_LOCK(&threadl);
	AST_LIST_TRAVERSE(&threadl, th, list) {
		if ((s->sin_family == th->tcptls_session->requestor.sin_family) &&
			(s->sin_addr.s_addr == th->tcptls_session->requestor.sin_addr.s_addr) &&
			(s->sin_port == th->tcptls_session->requestor.sin_port))  {
...
static int sip_prepare_socket(struct sip_pvt *p) 
{
	...
	if ((tcptls_session = sip_tcp_locate(&ca.sin))) {
...
static int sip_poke_noanswer(const void *data)
{
	...
	if (peer->lastms > -1) {
		...
		if (sip_cfg.peer_rtupdate) {
			ast_update_realtime(ast_check_realtime("sipregs") ? "sipregs" : "sippeers", "name", peer->name, "lastms", "-1", NULL);
		}
