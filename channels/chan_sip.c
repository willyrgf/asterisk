...
/* OEJ temporary hack */
#define UNLINKPACKET(element, head, prev) do {	\
	if (prev)				\
		(prev)->nextpacket = (element)->nextpacket;	\
	else					\
		(head) = (element)->nextpacket;	\
	} while (0)
...
static void add_blank(struct sip_message *req)
{
	if (!req->lines) {
...
static int send_response(struct sip_pvt *p, struct sip_message *req, enum xmittype reliable, int seqno)
{
	...
	if (p->do_history) {
		parse_request(req); 	/* We need to parse the request for history...  We whould add this to the message when creating it */
		append_history(p, reliable ? "TxRespRel" : "TxResp", "%s / %s - %s", req->parsedata->str, get_header(req, "CSeq"),
			(req->method == SIP_RESPONSE || req->method == SIP_UNKNOWN) ? REQ_OFFSET_TO_STR(req, rlPart2) : sip_methods[req->method].text);
	}
...
static int send_request(struct sip_pvt *p, struct sip_message *req, enum xmittype reliable, int seqno)
{
	...
	*/
	...
	if (p->do_history) {
		parse_request(req);
		append_history(p, reliable ? "TxReqRel" : "TxReq", "%s / %s - %s", req->data->str, get_header(req, "CSeq"), sip_methods[req->method].text);
	}
	res = (reliable) ?
...
static char *get_body(struct sip_message *req, char *name, char delimiter)
{
	int x;
	int len = strlen(name);
	char *r;
...
/*! \brief Parse a SIP message
	...
	Copies data to parsedata, leaving original data intact
*/
static int parse_request(struct sip_message *req)
{
	char *c;
	...
	if (!req->parsedata && !(req->parsedata = ast_str_create(SIP_MIN_PACKET))) {
		ast_log(LOG_ERROR, "Can't allocate memory for SIP parser.\n");
                return -1;
	}
	/* Copy req->data and keep req->data in raw format */
	ast_str_set(&req->parsedata, 0, "%s", req->data->str);
	ast_debug(1, "********** PARSING REQUEST !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
	c = req->parsedata->str;
	if (!*c)
		ast_debug(1,"-------------- NOTHING TO PARSE. SORRY\n");
	...
	for (; *c; c++) {
		if (*c == '\r') {		/* remove \r */
			*c = '\0';
			...
			ast_debug(1, "---- newline -");		/* ---- DEBUG ---- */
			*c = '\0';
			current_header_offset = (c + 1) - req->parsedata->str;
			previous_header = req->parsedata->str + dst[i];
			if (skipping_headers) {
				...
				ast_debug(1, "---- skipping -\n");		/* ---- DEBUG ---- */
				continue;
			}
			//if (sipdebug) {
				ast_debug(1, "%7s %2d [%3d]: %s\n",
	...
	*/
	...
	if (req->headers >= 0) {	/* we are in the body */
		...
		req->headers = i;
		req->lines = 0;
		/* req->parsedata->used will be a NULL byte */
		req->line[0] = ast_str_strlen(req->parsedata);
	}

	if (*c) {
		ast_log(LOG_WARNING, "Too many lines, skipping <%s>\n", c);
	}
	ast_debug(1, "********** DONE PARSING REQUEST !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
...
static int get_ip_and_port_from_sdp(struct sip_message *req, const enum media_type media, struct sockaddr_in *sin)
{
	const char *m;
	const char *c;
...
static int process_sdp(struct sip_pvt *p, struct sip_message *req, int t38action)
{
	...
	if (ast_test_flag(&p->flags[1], SIP_PAGE2_CALL_ONHOLD) && sin.sin_addr.s_addr && (!sendonly || sendonly == -1)) {
		...
		append_history(p, "Unhold", "%s", req->parsedata->str);
		if (sip_cfg.callevents)
		...
		append_history(p, "Hold", "%s", req->parsedata->str);
...
/*! \brief Generic initialization of SIP message structure, used for both request and response */
static int init_message(struct sip_message *message)
{
	memset(message, 0, sizeof(*message));
	if (!(message->data = ast_str_create(SIP_MIN_PACKET)))
		return -1;
	message->header[0] = 0;
	return 0;
}
...
/*! \brief Initialize SIP request for transmission  */
static int init_req(struct sip_message *message, int sipmethod, const char *ruri)
{
	...
	message->len = ast_str_strlen(message->data);
	message->headers++;
	return 0;
}
...
static int reqprep(struct sip_message *req, struct sip_pvt *p, int sipmethod, int seqno, int newbranch)
{
	struct sip_message *orig = &p->initreq;
	char stripped[80];
	char tmp[80];
	char newto[256];
	const char *c;
	...
	memset(req, 0, sizeof(struct sip_message));
...
static void copy_request(struct sip_message *dst, const struct sip_message *src)
{
	...
	struct ast_str *duplicate_parse = dst->parsedata;

	/* First copy stuff */
	memcpy(dst, src, sizeof(*dst));
	dst->data = duplicate;
	dst->parsedata = duplicate_parse;
	...
	if (src->parsedata) {
		if (!dst->parsedata && !(dst->parsedata = ast_str_create(src->parsedata->used + 1))) {
			return;
		} else if (dst->parsedata->len < src->parsedata->used + 1) {
			ast_str_make_space(&dst->parsedata, src->parsedata->used + 1);
		}
		memcpy(dst->parsedata->str, src->parsedata->str, src->parsedata->used + 1);
		dst->parsedata->used = src->parsedata->used;
	}
}
...
static int determine_firstline_parts(struct sip_message *req)
{
	char *e = ast_skip_blanks(req->parsedata->str);	/* there shouldn't be any */
	char *local_rlPart1;

	if (!*e)
		return -1;
	req->rlPart1 = e - req->parsedata->str;	/* method or protocol */
	local_rlPart1 = e;
	...
	if (!strcasecmp(local_rlPart1, "SIP/2.0") ) { /* We have a response */
		if (strlen(e) < 3)	/* status code is 3 digits */
			return -1;
		req->rlPart2 = e - req->parsedata->str;
		...
		req->rlPart2 = e - req->parsedata->str;	/* URI */
		e = ast_skip_nonblanks(e);
		if (*e)
			*e++ = '\0';
		e = ast_skip_blanks(e);
 ...
 */
static void add_diversion_header(struct sip_message *req, struct sip_pvt *pvt)
{
...
static enum parse_register_result parse_register_contact(struct sip_pvt *pvt, struct sip_peer *peer, struct sip_message *req)
{
	char contact[SIPBUFSIZE];
 ...
 */
static int get_pai(struct sip_pvt *p, struct sip_message *req)
{
	char pai[256];
	char privacy[64];
	char *cid_num = "";
...
{
	char tmp[256] = "", *c, *a;
	struct sip_message *req = oreq ? oreq : &p->initreq;
 ...
 */
static attribute_unused void check_via_response(struct sip_pvt *p, struct sip_message *req)
{
	char via[256];
	char *cur, *opts;
...
static enum check_auth_result check_peer_ok(struct sip_pvt *p, char *of,
	struct sip_message *req, int sipmethod, struct sockaddr_in *sin,
...
static void handle_request_info(struct sip_pvt *p, struct sip_message *req)
{
	...
	transmit_response(p, 415, "415 Unsupported media type", req);
	return;
}
...
static int do_register_auth(struct sip_pvt *p, struct sip_message *req, enum sip_auth_type code)
{
...
static int do_proxy_auth(struct sip_pvt *p, struct sip_message *req, enum sip_auth_type code, int sipmethod, int init)
{
	char *header, *respheader;
	char digest[1024];
...
static int read_to_parts(struct sip_pvt *p, struct sip_message *req, char **name, char **number)
{
 ...
 */
static void handle_response_update(struct sip_pvt *p, int resp, const char *rest, struct sip_message *req, int seqno)
{
	if (p->options) {
...
static void handle_response_peerpoke(struct sip_pvt *p, int resp, struct sip_message *req)
{
...
static void *sip_park_thread(void *stuff)
{
	...
	if (ast_do_masquerade(transferee)) {
		...
		transmit_response(transferer->tech_pvt, 503, "503 Internal error", &req);
	...
	transmit_response(transferer->tech_pvt, 202, "202 Accepted", &req);
	if (!res)	{
		/* Transfer succeeded */
...
static int handle_request_notify(struct sip_pvt *p, struct sip_message *req, struct sockaddr_in *sin, int seqno, const char *e)
{
	...
	if (!strcmp(event, "refer")) {
		...
		if (strncasecmp(get_header(req, "Content-Type"), "message/sipfrag", strlen("message/sipfrag"))) {
			/* We need a sipfrag */
			transmit_response(p, 400, "400 Bad request", req);
			sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
			return -1;
		}
		...
		*sep++ = '\0';			/* Response string */
		...
		transmit_response(p, 489, "489 Bad event", req);
		res = -1;
	}
...
static int handle_request_options(struct sip_pvt *p, struct sip_message *req)
{
	...
	*/
	...
	if (p->lastinvite) {
		...
		transmit_response_with_allow(p, 200, "200 OK", req, 0);
		return 0;
	}
	...
	if (ast_shutting_down())
		transmit_response_with_allow(p, 503, "503 Unavailable", req, 0);
	else if (res < 0)
		transmit_response_with_allow(p, 404, "404 Not Found", req, 0);
	else
		transmit_response_with_allow(p, 200, "200 OK", req, 0);
...
static int handle_invite_replaces(struct sip_pvt *p, struct sip_message *req, int debug, int seqno, struct sockaddr_in *sin)
{
	...
	if (req->ignore) {
		...
		/* We should answer something here. If we are here, the
			...
			can't harm */
		transmit_response_with_sdp(p, 200, "200 OK", req, XMIT_RELIABLE, FALSE, FALSE);
	...
	if (!c) {
		...
		transmit_response_reliable(p, 503, "503 Service Unavailable", req);
...
static int handle_request_update(struct sip_pvt *p, struct sip_message *req)
{
	if (ast_strlen_zero(get_header(req, "X-Asterisk-rpid-update"))) {
		transmit_response(p, 501, "501 Method Not Implemented", req);
		return 0;
	}
	if (get_rpid(p, req)) {
		struct ast_party_connected_line connected;
...
static int handle_request_invite(struct sip_pvt *p, struct sip_message *req, int debug, int seqno, struct sockaddr_in *sin, int *recount, const char *e, int *nounlock)
{
	...
	if (!ast_strlen_zero(required)) {
		...
		if (required_profile && !(required_profile & (SIP_OPT_REPLACES | SIP_OPT_TIMER))) {
			...
			transmit_response_with_unsupported(p, 420, "420 Bad extension (unsupported)", req, required);
	...
	if (ast_test_flag(&p->flags[0], SIP_OUTGOING) && p->owner && (p->invitestate != INV_TERMINATED && p->invitestate != INV_CONFIRMED)) {
		...
		if (!different) {
			transmit_response(p, 482, "482 Loop Detected", req);
			p->invitestate = INV_COMPLETED;
			sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
			return 0;
		} else {
	...
	if (!req->ignore && p->pendinginvite) {
		if (!ast_test_flag(&p->flags[0], SIP_OUTGOING) && (p->invitestate == INV_COMPLETED || p->invitestate == INV_TERMINATED)) {
			...
			transmit_response_reliable(p, 491, "491 Request Pending", req);
	...
	if (!ast_strlen_zero(p_replaces)) {
		...
		if (p->owner) {
			...
			transmit_response_reliable(p, 400, "400 Bad request", req);	/* The best way to not not accept the transfer */
		...
		if (!p->refer && !sip_refer_allocate(p)) {
			transmit_response_reliable(p, 500, "500 Server Internal Error", req);
		...
		*/
		...
		if (strncmp(replace_id, "pickup-", 7) == 0) {
			...
			if ((subscription = get_sip_pvt_byid_locked(replace_id, NULL, NULL)) == NULL) {
				...
				transmit_response_reliable(p, 481, "481 Call Leg Does Not Exist (Replaces)", req);
				error = 1;
			} else {
		...
		if (!error && ast_strlen_zero(pickup.exten) && !p->refer->refer_call->owner) {
			...
			transmit_response_reliable(p, 481, "481 Call Leg Does Not Exist (Replace)", req);
			error = 1;
		}
		...
		if (!error && ast_strlen_zero(pickup.exten) && p->refer->refer_call->owner->_state != AST_STATE_RINGING && p->refer->refer_call->owner->_state != AST_STATE_RING && p->refer->refer_call->owner->_state != AST_STATE_UP) {
			...
			transmit_response_reliable(p, 603, "603 Declined (Replaces)", req);
			error = 1;
		}
		...
		if (!p->owner) {	/* Not a re-invite */
			...
			if (find_sdp(req)) {
				if (process_sdp(p, req, SDP_T38_INITIATE)) {
					transmit_response_reliable(p, 488, "488 Not acceptable here", req);
					if (!p->lastinvite)
						sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
					return -1;
				}
	...
	if (!p->lastinvite && !req->ignore && !p->owner) {
		...
		if (res < 0) { /* Something failed in authentication */
			if (res == AUTH_FAKE_AUTH) {
				...
				transmit_response_reliable(p, 403, "403 Forbidden", req);
			}
			p->invitestate = INV_COMPLETED;	
			sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
			ast_string_field_set(p, theirtag, NULL);
		...
		if ((res = update_call_counter(p, INC_CALL_LIMIT))) {
			if (res < 0) {
				...
				transmit_response_reliable(p, 480, "480 Temporarily Unavailable (Call limit) ", req);
				sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
				p->invitestate = INV_COMPLETED;	
			}
			return 0;
		...
		if (!replace_id && gotdest) {	/* No matching extension found */
			if (gotdest == 1 && ast_test_flag(&p->flags[1], SIP_PAGE2_ALLOWOVERLAP))
				transmit_response_reliable(p, 484, "484 Address Incomplete", req);
			else {
				char *decoded_exten = ast_strdupa(p->exten);
				
				transmit_response_reliable(p, 404, "404 Not Found", req);
				ast_uri_decode(decoded_exten);
	...
	if (p->sipoptions & SIP_OPT_TIMER) {
		...
		if (!ast_strlen_zero(p_uac_se_hdr)) {
			...
			if (rtn != 0) {
				transmit_response_reliable(p, 400, "400 Session-Expires Invalid Syntax", req);
				p->invitestate = INV_COMPLETED;
				if (!p->lastinvite) {
					sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
				}
		...
		if (!ast_strlen_zero(p_uac_min_se)) {
			...
			if (rtn != 0) {
        			transmit_response_reliable(p, 400, "400 Min-SE Invalid Syntax", req);
       	   			p->invitestate = INV_COMPLETED;
       	   			if (!p->lastinvite) {
					sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
				}
		...
		case SESSION_TIMER_MODE_ORIGINATE:
			if (uac_max_se > 0 && uac_max_se < dlg_min_se) {
				transmit_response_with_minse(p, 422, "422 Session Interval Too Small", req, dlg_min_se);
				p->invitestate = INV_COMPLETED;
				if (!p->lastinvite) {
					sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
				}
		...
		case SESSION_TIMER_MODE_REFUSE:
			if (p->reqsipoptions & SIP_OPT_TIMER) {
				transmit_response_with_unsupported(p, 420, "420 Option Disabled", req, required);
	...
	if (c) {	/* We have a call  -either a new call or an old one (RE-INVITE) */
		...
		case AST_STATE_DOWN:
			...
			transmit_provisional_response(p, 100, "100 Trying", req, 0);
			p->invitestate = INV_PROCEEDING;
			ast_setstate(c, AST_STATE_RING);
			if (strcmp(p->exten, ast_pickup_ext())) {	/* Call to extension -start pbx on this call */
				...
				case AST_PBX_FAILED:
					...
					p->invitestate = INV_COMPLETED;
					transmit_response_reliable(p, 503, "503 Unavailable", req);
					break;
				case AST_PBX_CALL_LIMIT:
					ast_log(LOG_WARNING, "Failed to start PBX (call limit reached) \n");
					p->invitestate = INV_COMPLETED;
					transmit_response_reliable(p, 480, "480 Temporarily Unavailable", req);
					break;
		...
		case AST_STATE_RING:
			...
			break;
		case AST_STATE_RINGING:
			transmit_provisional_response(p, 180, "180 Ringing", req, 0);
			p->invitestate = INV_PROCEEDING;
			break;
		case AST_STATE_UP:
			...
			if (p->t38.state == T38_PEER_REINVITE) {
				...
				ast_set_flag(&p->flags[1], SIP_PAGE2_DIALOG_ESTABLISHED);
				transmit_response_with_t38_sdp(p, 200, "200 OK", req, (reinvite ? XMIT_RELIABLE : (req->ignore ?  XMIT_UNRELIABLE : XMIT_CRITICAL)));
			} else if (p->t38.state == T38_DISABLED) {
				/* If this is not a re-invite or something to ignore - it's critical */
				ast_set_flag(&p->flags[1], SIP_PAGE2_DIALOG_ESTABLISHED);
				transmit_response_with_sdp(p, 200, "200 OK", req, (reinvite ? XMIT_RELIABLE : (req->ignore ?  XMIT_UNRELIABLE : XMIT_CRITICAL)), p->session_modify == TRUE ? FALSE : TRUE, FALSE);
			}
		...
		if (p && (p->autokillid == -1)) {
			...
			transmit_response_reliable(p, respcode, msg, req);
			p->invitestate = INV_COMPLETED;
			sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
		}
	}
...
static int local_attended_transfer(struct sip_pvt *transferer, struct sip_dual *current, struct sip_message *req, int seqno)
{
	...
	if (!(targetcall_pvt = get_sip_pvt_byid_locked(transferer->refer->replaces_callid, transferer->refer->replaces_callid_totag,
		...
		if (transferer->refer->localtransfer) {
			...
			transmit_response(transferer, 202, "202 Accepted", req);
...
static int handle_request_refer(struct sip_pvt *p, struct sip_message *req, int debug, int seqno, int *nounlock)
{
	...
	if (!p->owner) {
		...
		transmit_response(p, 603, "603 Declined (No dialog)", req);
		if (!req->ignore) {
	...
	if (p->allowtransfer == TRANSFER_CLOSED ) {
		...
		transmit_response(p, 603, "603 Declined (policy)", req);
	...
	if (!req->ignore && ast_test_flag(&p->flags[0], SIP_GOTREFER)) {
		...
		transmit_response(p, 491, "491 Request pending", req);
	...
	if (!p->refer && !sip_refer_allocate(p)) {
		transmit_response(p, 500, "500 Internal Server Error", req);
	...
	*/
	...
	if (!current.chan2 && !p->refer->attendedtransfer) {
		...
		transmit_response(p, 603, "603 Declined", req);
		return -1;
	}
...
static int handle_request_cancel(struct sip_pvt *p, struct sip_message *req)
{
	...
	if (p->initreq.len > 0) {
		...
		AST_LIST_TRAVERSE_SAFE_END;
		transmit_response_reliable(p, 487, "487 Request Terminated", &p->initreq);
		transmit_response(p, 200, "200 OK", req);
		return 1;
	} else {
		transmit_response(p, 481, "481 Call Leg Does Not Exist", req);
		return 0;
	}
}
...
static int handle_request_subscribe(struct sip_pvt *p, struct sip_message *req, struct sockaddr_in *sin, int seqno, const char *e)
{
	...
	if (p->initreq.headers) {	
		...
		if (p->initreq.method != SIP_SUBSCRIBE) {
			...
			/* For transfers, this could happen, but since we haven't seen it happening, let us just refuse this */
 			transmit_response(p, 403, "403 Forbidden (within dialog)", req);
	...
	*/
	if (!sip_cfg.allowsubscribe) {
 		transmit_response(p, 403, "403 Forbidden (policy)", req);
	...
	if (!req->ignore && !resubscribe) {	/* Set up dialog, new subscription */
		...
		if (!ast_strlen_zero(to) && gettag(req, "To", totag, sizeof(totag))) {
			...
			transmit_response(p, 481, "481 Subscription does not exist", req);
	...
	build_contact(p);
	if (gotdest) {
		transmit_response(p, 404, "404 Not Found", req);
	...
	if (!strcmp(event, "presence") || !strcmp(event, "dialog")) { /* Presence, RFC 3842 */
		...
		if (pidf_xml && strstr(p->useragent, "Polycom")) {
			...
			transmit_response(p, 489, mybuf, req);
		...
		if (!ast_strlen_zero(acceptheader) && strcmp(acceptheader, "application/simple-message-summary")) {
			...
			transmit_response(p, 406, "406 Not Acceptable", req);
		...
		*/
		if (!authpeer || AST_LIST_EMPTY(&authpeer->mailboxes)) {
			transmit_response(p, 404, "404 Not found (no mailbox)", req);
			pvt_set_needdestroy(p, "received 404 response");
...
static int handle_incoming(struct sip_pvt *p, struct sip_message *req, struct sockaddr_in *sin, int *recount, int *nounlock)
{
	...
	if (sip_cfg.pedanticsipchecking) {
		...
		if (!p->initreq.headers && req->has_to_tag) {
			...
			if (!req->ignore && req->method == SIP_INVITE) {
				transmit_response_reliable(p, 481, "481 Call/Transaction Does Not Exist", req);
				/* Will cease to exist after ACK */
			} else if (req->method != SIP_ACK) {
				transmit_response(p, 481, "481 Call/Transaction Does Not Exist", req);
				sip_scheddestroy(p, DEFAULT_TRANS_TIMEOUT);
			} else {
	...
	case SIP_ACK:
		...
		break;
	default:
		transmit_response_with_allow(p, 501, "501 Method Not Implemented", req, 0);
...
static int queue_request(struct sip_pvt *p, const struct sip_message *req)
{
	struct sip_message *newreq;
...
static int handle_request_do(struct sip_message *req, struct sockaddr_in *sin)
{
	...
	if (p->do_history) /* This is a request or response, note what it was for */
		append_history(p, "Rx", "%s / %s / %s", req->parsedata->str, get_header(req, "CSeq"), REQ_OFFSET_TO_STR(req, rlPart2));
...
static int sip_sipredirect(struct sip_pvt *p, const char *dest)
{
	...
	transmit_response_reliable(p, 302, "302 Moved Temporarily", &p->initreq);
