...
struct sip_peer {
	...
	char failoverhostname[MAXHOSTNAMELEN]; /*!< failover host name - maybe should be a stringfield :-) OEJ */
	...
	if (pkt->retrans < MAX_RETRANS) {
		...
		if (sip_debug_test_pvt(pkt->owner)) {
			...
			ast_verbose("Retransmitting #%d (%s) to %s%s:%d:\n%s\n---\n",
				...
				//pkt->owner->failoverstate == ACTIVE ? ntohs(pkt->owner->failoveraddr.sin_port) : ntohs(dst->sin_port), pkt->data);
		}
...
static int transmit_register(struct sip_registry *r, int sipmethod, const char *auth, const char *authheader)
{
	...
	if (r->call) {	/* We have a registration */
		...
		if (create_addr(p, r->hostname, &r->us, FALSE)) {
