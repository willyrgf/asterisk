		...
		if (prunepeer) {
			if ((device = ASTOBJ_CONTAINER_FIND_UNLINK(&peerl, name))) {
				if (!ast_test_flag((&device->flags[1]), SIP_PAGE2_RTCACHEFRIENDS)) {
					ast_cli(fd, "User '%s' is not a Realtime peer, cannot be pruned.\n", name);
...
static void set_device_defaults(struct sip_peer *device)
{
	...
	ast_copy_flags(&device->flags[0], &global_flags[0], SIP_FLAGS_TO_COPY);
	ast_copy_flags(&device->flags[1], &global_flags[1], SIP_PAGE2_FLAGS_TO_COPY);
	...
	strcpy(device->mohinterpret, default_mohinterpret);
	strcpy(device->mohsuggest, default_mohsuggest);
	...
	device->allowtransfer = global_allowtransfer;
...
static int unload_module(void)
{
	...
	ASTOBJ_CONTAINER_DESTROYALL(&peerl, sip_destroy_device);
